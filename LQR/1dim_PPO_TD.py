...
def compare_paths(x_sim,x_star,ylabel):
    ...
    ax.plot(t,x_sim,color=colors[1],label='Agent')
    ax.plot(t,x_star,color=colors[0],label='True')
...
class PPO:
    ...
    def update_critic(self, memory):   
        ...
        # update w/ scaled TD(0) targets
        td_targets = torch.squeeze(torch.stack(memory.td_targets)).to(device).detach()
        
        # Optimize critic for K epochs:
        for _ in range(self.K_epochs):
            state_values = self.alpha*torch.squeeze(self.policy.critic(states))
            
            critic_loss = 0.5/(self.alpha**2)*self.MseLoss(state_values,td_targets)
...
if __name__ == '__main__':
    ...
    K_epochs = 10                # update for K epochs
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        for t in range(max_timesteps):
            ...
            # Saving cost, scaled td_targets, and is_terminals:
            memory.costs.append(cost.item())
            memory.td_targets.append(cost.item() + alpha*gamma*ppo.policy.critic(torch.as_tensor(state).float().to(device).detach()))
