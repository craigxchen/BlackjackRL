...
def train(K,model2):
    ...
    for i in range(NUM_TRIALS):
        ...
        if (i+1)%(NUM_TRIALS/10) == 0:
            ...
            print('x = '+str(x))
            print('y = '+str(model2(np.array(1).reshape(1,1))))
        ...
        for t in range(T):
            ...
            y = r + ALPHA*GAMMA*model2(np.matmul(A,x) + np.matmul(B,u))

            y_hat = model2.net_forward(x)

            lr = 0.001

            total_loss += loss(y, y_hat, ALPHA)
            model2.net_backward(y, y_hat, ALPHA)
            model2.update_wb(lr)
